                      ...
                      const LutSrvGetter& getLutSRV)
{
    ...
    if (!agents.empty())
    {
        ...
        for (auto& a : agents)
        {
            if (Team::IsBlue(a.teamId)) blue.push_back(&a);
            else if (Team::IsRed(a.teamId)) red.push_back(&a);
        }
    ...
    else
    {
        ...
        for (size_t i = 0; i < settings.layers.size(); ++i)
        {
            ...
            {
                if (layer.subjectType == HeatmapSubjectType::TEAM && Team::IsBlue(layer.subjectId))
                    ImGui::PushStyleColor(ImGuiCol_Text, kBlueText);
                else if (layer.subjectType == HeatmapSubjectType::TEAM && Team::IsRed(layer.subjectId))
                    ImGui::PushStyleColor(ImGuiCol_Text, kRedText);
